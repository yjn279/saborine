...
プッシュ通知はiOS版でのみ提供する。Web版は配布の容易さ(URLを送るだけで使える)を担う検証チャネルであり、通知はアプリ内バナーで代替する(iOSのWeb通知は「ホーム画面に追加」が前提となり、テスト協力者への負担と実装コストに見合わない)。
...
| 2 | 転換仮説: 感謝を送るだけで始めた「しない側」が、7日以内に自分から家事を記録する | 実装版アプリで1週間の生活テストを実カップル5組で行う(Web版をURLで配布。プッシュ通知はアプリ内バナー+運営からの連絡で代替する)。見た目だけのモックでは検証できない | 5組中3組以上で、しない側が7日以内に自発的な記録を1回以上行う。途中離脱は1組以下 |
