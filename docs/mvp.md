...
- 招待はキャラクターからの手紙にする。文面は「はじめまして。ぼくにはまだ里親がひとりしかいません。もうひとりの里親になってくれませんか」であり、彩花のお願いではなくキャラクターのお願いという構図にする
...
1. 促しの主語と場所を限定する。促すのはキャラクターだけ、場所はアプリ内のセリフだけとする。通知で促さないため、催促と受け取られる余地がない
2. 1回の価値を非対称にする。後述の成長計算式により、貢献が少ない側のごはんは多い側の3倍の栄養になる。「少ないことが責められる理由」ではなく「自分の1回が最も効く理由」に反転する
3. 記録のハードルを最小にする。プリセットから1タップ、写真・詳細・相手の承認は不要とする
...
| 週次カード | 1週間の物語をふたりで見る | 物語カード、保存、共有 |
...
| V2以降 | 家事プリセットの編集・並べ替え | MVPはプリセット+自由入力1枠で足りる |
...
| 2 | 転換仮説: 感謝を送るだけで始めた「しない側」が、7日以内に自分から家事を記録する | 見た目だけのモックを配り、通知やキャラクターの反応は運営が裏で手動送信する1週間の生活テストを実カップル5組で行う | 5組中3組以上で、しない側が7日以内に自発的な記録を1回以上行う。途中離脱は1組以下 |
...
| 5 | 需要・キャラクター仮説: このコンセプトとサボテン案には、事前登録するだけの需要がある | 紹介ページをサボテン版と動物版の2種類作り、SNS広告から流入させて事前登録率を比べる | 訪問から事前登録への転換率5%以上。あわせて勝った方をキャラクターの方向性として決定する |
