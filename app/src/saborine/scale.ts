...
// 育ち具合が0未満・1を超える入力(防御)は0〜1に丸め、最大の段階(3)は超えない。
export function calcScale(evolutionStage: number, growthProgress: number = 0): number {
  const clampedProgress = Math.min(Math.max(growthProgress, 0), 1);
  const stage = Math.min(evolutionStage + clampedProgress, MAX_SCALE_STAGE);
